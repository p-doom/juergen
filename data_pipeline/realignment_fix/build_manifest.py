#!/usr/bin/env python3
"""Build a manifest over a raw crowd-cast uploads tree.
    PYTHONPATH=. python3 -m realignment_fix.build_manifest \
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--workers", type=int, default=32,
                   help="worker processes for parallel cv2 probes")
...
def main() -> None:
    ...
    with mp.Pool(args.workers) as pool:
        # cv2 probes are independent per file; imap_unordered fans them across
        # processes and yields each as it finishes (deterministic order is
        # restored by the sort below). chunksize amortizes the IPC round-trip
        # over these cheap metadata reads. mininterval throttles redraws so a
        # slurm log isn't spammed; the postfix tracks the skip tallies live.
        bar = tqdm(
            pool.imap_unordered(build_row, videos, chunksize=8),
            total=len(videos), unit="vid", desc="[manifest] probe",
            smoothing=0.05, mininterval=2.0, dynamic_ncols=True,
        )
        for row in bar:
            # Refresh the tally postfix each tick (the bar's count/rate/ETA
            # already advance on every next(), before any continue below).
            bar.set_postfix(
                kept=len(rows), bad_name=n_bad_name,
                bad_video=n_bad_video, no_keylog=n_no_keylog, refresh=False,
            )
            if row is None:
                n_bad_name += 1
                continue
