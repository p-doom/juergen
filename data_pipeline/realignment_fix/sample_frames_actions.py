...
``stage_01_frames_actions``: given a ``--target-fps`` (<= master) it
  ...
  * applies the same NO_OP head/tail thinning as stage 01, and
...
def _is_cached(s01_dir: Path, target_fps: float, head: int, tail: int) -> list[dict[str, Any]] | None:
    """Return existing records iff a prior run wrote them at the SAME params
    (so re-sampling a different fps into the same dir never silently mixes)."""
...
def sample_segment(task: dict[str, Any]) -> dict[str, Any]:
    ...
    head = int(task["noop_keep_head"])
    tail = int(task["noop_keep_tail"])
    ...
    if not task["force"]:
        cached = _is_cached(s01_dir, target_fps, head, tail)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--noop-keep-tail", type=int, default=config.DEFAULT_NOOP_KEEP_TAIL,
                   help="Within each NO_OP run, keep the last this many frames.")
...
def main() -> None:
    ...
    print(
        ...
        f"(master_fps={master_fps}) noop_keep={args.noop_keep_head}/{args.noop_keep_tail} "
        f"| workers={n_workers}",
        flush=True,
    )
    ...
    print(
        ...
        f"({n_non_noop_total} non-NO_OP) -> {out_dir}",
        flush=True,
    )
