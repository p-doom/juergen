"""Stage F: compile per-split chat.jsonl into Grain ArrayRecord shards.
...
def _run_split(split: str, src_chat: Path, out_split_dir: Path) -> dict:
    ...
    print(f"[stage_f] {split}: {' '.join(cmd)}", flush=True)
    t0 = time.time()
...
def main(_) -> None:
    ...
    for split in ("train", "val", "test"):
        ...
        if not src_chat.is_file():
            print(f"[stage_f] no chat.jsonl for split {split}, skipping")
            continue
