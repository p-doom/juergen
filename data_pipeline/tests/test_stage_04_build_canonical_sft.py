    ...
    def test_rewrites_stage03_outputs_into_canonical_artifact(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            manifest = build_canonical_sft(
                ...
                system_prompt_text="labctl prompt uses <TERMINATED>",
                terminal_token="<TERMINATED>",
                terminal_mode="replace_final_assistant",
            )
            ...
            for record in records:
                ...
                self.assertEqual(messages[0]["content"][0]["text"], "labctl prompt uses <TERMINATED>")
                ...
                image_block = next(
                    block for block in first_user["content"] if block["type"] == "image"
                )
                image_path = Path(image_block["image"])
                self.assertTrue(image_path.is_absolute())
                self.assertTrue(image_path.is_file())
                self.assertEqual(messages[-1]["content"], [{"type": "text", "text": "<TERMINATED>"}])

            self.assertTrue(all(len(splits) == 1 for splits in groups_by_split.values()))
            self.assertFalse((out_dir / "images").exists())
            self.assertEqual(manifest["image_path_mode"], "absolute")
            self.assertEqual(manifest["message_policy"]["owner"], "labctl")
            self.assertEqual(
                {"train", "val"},
