...
combos, fully-inside pair discard, delta discards, conservation, and
...
def _format(events, windows=WINDOWS_A, zones=ZONES_A):
    return format_segment(events, windows, zones, master_fps=M).labels
...
class StraddleClampTest(unittest.TestCase):
    def test_release_in_zone_clamps_to_zone_start(self) -> None:
        labels = _format(_events((6.0, "press", "KeyA"), (12.0, "release", "KeyA")))
    ...
    def test_pair_fully_inside_zone_is_dropped(self) -> None:
        labels = _format(_events((10.5, "press", "KeyD"), (11.5, "release", "KeyD")))
    ...
    def test_unreleased_press_in_zone_is_dropped(self) -> None:
        ...
        labels = _format(_events((11.0, "press", "KeyI")))
        self.assertEqual(labels, ["NO_OP", "NO_OP", "NO_OP"])
        _assert_balanced(self, labels)

    def test_unreleased_visible_press_is_dropped(self) -> None:
        labels = _format(_events((6.0, "press", "KeyJ")))
        self.assertEqual(labels, ["NO_OP", "NO_OP", "NO_OP"])
...
class StateLayerTest(unittest.TestCase):
    def test_full_stream_annotated_and_conserved(self) -> None:
        ...
        labeled = apply_label_policy(events, WINDOWS_A, ZONES_A, master_fps=M)
        ...
        self.assertEqual(labeled[2].discard_reason, "redundant_press")
        self.assertEqual(labeled[3].discard_reason, "dangling_release")
        self.assertEqual(labeled[4].discard_reason, "black")
