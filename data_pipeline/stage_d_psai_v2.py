"""PSAI golden SFT ingest, v2 terminate-gated variant of stage_d_psai_v1.

Mirrors stage_d_videocua_v2's gating: rewrite terminate only on the
episode-final sub-record and only when at least min_preterminate_actions
non-NO_OP assistant turns precede the final turn. Uses native mouse deltas,
with no IDM.
"""
...
import stage_d_videocua_v2 as videocua_v2
...
def main(_) -> None:
    ...
    print(
        f"[stage_d_psai v2] no_op cap k_frames={k_frames} "
    ...
    for r in capped:
        out, status = videocua_v2._transform_record_terminate_gated(
            r,
    ...
    print(
        f"[stage_d_psai v2] terminate gating: rewritten {term_stats['rewritten']} "
    ...
    print(
        f"[stage_d_psai v2] {len(transformed)} records → "
    ...
    for split, records in (("train", train_records), ("val", val_records)):
        if not records:
            print(f"[stage_d_psai v2] no records for split {split}, skipping")
            continue
    ...
    write_manifest(
        output_dir,
        stage="chunk_index_psai_v2_no_op_capped_terminate_gated",
        params={
