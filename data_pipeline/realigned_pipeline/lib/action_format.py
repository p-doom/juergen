...
``plan_typing_coalesce`` extends that collapse ACROSS windows: it decides which
runs of typing-only windows stage 04 may fuse into a single turn (dropping the
later windows' frames). It only PLANS — the caller re-derives the labels from
merged windows, which is what turns per-window ``type("ab")`` + ``type("cd")``
into one ``type("abcd")`` and what repairs key rollover / held-Shift spans that
no single window could balance.
...
class FormatResult:
    ...
    # The RENDERED primitives behind ``labels``, one list per window (None for
    # formats that don't render primitives). Exposed so a caller can classify a
    # window structurally instead of re-parsing the label text — a ``type("a; b")``
    # payload contains the primitive separator, so label strings are not safely
    # splittable. Consumed by ``plan_typing_coalesce``.
    primitives: list[list[ActionPrimitive]] | None = None
...
def is_typing_only_window(
    prims: Sequence[ActionPrimitive], held_non_shift_mods: Sequence[str] | set[str] = ()
) -> bool:
    ...
    return all(
        ...
        or (
            p.kind in ("down", "up")
            and (p.input_name in _US_PRINTABLE or p.input_name in _SHIFT_KEYS)
        )
        for p in prims
    )
...
def plan_typing_coalesce(
    primitives: Sequence[Sequence[ActionPrimitive]],
    *,
