...
(integer stride required). The master axis is integer ticks == master record
indices; nothing here touches timestamps.

Selector semantics (load-bearing):
  * stride = master_fps / fps must be an integer; slot j -> ideal tick
    ``j * stride``. If that tick is masked by the filter, the slot yields NO
    frame — skip, never substitute a neighbor.
...
from realigned_pipeline.lib.events import DeadZone, Window
from realigned_pipeline.lib.image_store import make_arrayrecord_image_uri
from realigned_pipeline.lib.manifest import check_artifact_id, make_artifact_id
...
def resolve_stride(master_fps: float, fps: float) -> int:
    """Integer tick stride for sampling at ``fps`` from a ``master_fps`` axis.

    Raises for fps > master or non-integer ratios: 5 fps on a 4 fps master (or
    0.4 fps on 15) has no exact tick alignment and must fail loudly."""
    if fps <= 0:
...
class ViewFrame:
    ...
    slot: int  # ideal-slot index j (master_idx == j * stride)
    master_idx: int
...
def build_segment_view(filter_seg: dict[str, Any], *, fps: float) -> SegmentView:
    ...
    n_slots = (n_records + stride - 1) // stride
...
class FilterArtifact:
    ...
    def iter_views(self, fps: float, *, limit: int | None = None) -> Iterator[SegmentView]:
        self.stride_for(fps)  # validate once up front
        rows = self.usable_rows()
        if limit is not None:
            rows = rows[:limit]
        for row in rows:
            yield self.segment_view(str(row["segment_id"]), fps)
