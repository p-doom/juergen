...
def _detect_action_format(ds: Any, path: Path) -> "str | None":
    ...
    artifact manifest. ``None`` for a stage-01b frame_records or frames-master
...
function render(d){
  ...
  if(d.n_frames===0){ $('#grid').innerHTML='<div class="empty">no action tokens in this dataset (a frames-master store is keylog-free — run stage 01b / 04 to get actions).</div>'; $('#tiles').innerHTML=''; $('#movbar').style.display='none'; $('#fmtnote').textContent=''; return; }
