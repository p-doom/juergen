...
across the whole dataset:
  ...
  * What exact mouse movements dominate (e.g. is ``-100 10 0`` over-represented)?
    ...
    from the moves that happen while the wheel turns.
...
def _new_move_acc() -> dict[str, Any]:
    ...
    return {
        "n_move": 0,       # frames with dx|dy != 0
        "n_rows": 0,       # frames feeding the exact-triple list (move / scroll / keys)
        "triples": Counter(),
        "directions": Counter(),
        ...
        for a in actions:
            ...
            for acc in (move_acc["all"],
                ...
                if moved or has_scroll or events:
                    acc["n_rows"] += 1
                    # a nonzero hscroll is shown in the extended 4-number form so a
                    # horizontal-scroll frame doesn't masquerade as a bare "0 0 0"
...
function render(d){
  ...
  if(d.n_frames===0){ $('#grid').innerHTML='<div class="empty">no action tokens in this dataset (a frames-master store is keylog-free — run stage 01b / 04 to get actions).</div>'; $('#tiles').innerHTML=''; $('#movbar').style.display='none'; return; }
...
function renderGrid(d){
  ...
  cards.push(barCard('top mouse movements',`dx dy scroll (any keys ignored) · ${vsub}`,
