...
and ``disp`` collapses the stream into readable chips — typing runs gathered
into ``type("…")`` (rollover-tolerant, Shift folded), click pairs, chorded
keys — while the verbatim text stays in ``action``.
...
# movement/scroll into the ``format_action`` head; ``disp`` collapses the
# per-event stream into readable chips.
# --------------------------------------------------------------------------- #
...
_ORDERED_SHIFT = frozenset({"ShiftLeft", "ShiftRight"})
# Non-Shift modifiers veto typing runs (Ctrl+C is a chord, not typing) — the
# formatter's _NON_SHIFT_MODIFIERS set.
_ORDERED_MODS = _ORDERED_SHIFT | frozenset({
    "ControlLeft", "ControlRight", "Control",
    "Alt", "AltLeft", "AltRight", "AltGr",
    "MetaLeft", "MetaRight", "Meta", "MetaGr", "Function",
})
_ORDERED_BUTTONS = {"LMB": "click", "RMB": "rclick", "MMB": "mclick"}


def _ordered_typing_char(name: str, shift: bool) -> "str | None":
    """The character ``down(name)`` types, or None for non-typing keys. Return
    and Tab are down/up in the ordered grammar, never typed characters."""
    if name in ("Return", "Enter", "NumpadEnter", "Tab"):
        return None
    return _key_to_char(name, shift)
...
def _ordered_disp(prims: "list[tuple[Any, ...]]", held: "dict[str, str]") -> list[str]:
    """Collapse one turn's ordered primitives into compact display chips.

    Contiguous move/scroll runs sum into one chip; typed characters (taken at
    ``down`` in stream order, so key rollover still reads as text; Shift folded
    into the char) gather into one ``type("…")`` even across interleaved
    scrolling; adjacent button pairs become click/dblclick/triclick; a key
    pressed under held non-Shift modifiers chips as the chord
    (``ControlLeft+key(KeyC)``). ``held`` is the cross-turn modifier state
    (name -> "carried" | "new" | "used"), updated in place: a modifier only
    chips when tapped bare (``key(ShiftLeft)``), released from an earlier turn
    (``up(…)``) or still unused-held at turn end (``down(…)``)."""
    chips: list[str] = []
    pend_kind: "str | None" = None  # coalescing move/scroll accumulator
    pend_dx = pend_dy = 0
    buf: list[str] = []  # typed characters gathered across the turn

    def flush_pend() -> None:
        nonlocal pend_kind, pend_dx, pend_dy
        if pend_kind is not None and (pend_dx or pend_dy):
            chips.append(f"{pend_kind}({pend_dx},{pend_dy})")
        pend_kind, pend_dx, pend_dy = None, 0, 0

    def flush_buf() -> None:
        if buf:
            t = "".join(buf)
            short = t if len(t) <= 40 else t[:37] + "…"
            chips.append("type(" + json.dumps(short) + ")")
            buf.clear()

    def chord_prefix() -> str:
        """Held modifiers as a ``Mod+`` prefix; marks them chord-used."""
        for m in held:
            if held[m] == "new":
                held[m] = "used"
    ...
    while i < n:
        ...
        if kind in ("move", "scroll"):
            ...
            i += 1
            continue
        if kind == "type":  # v3 collapsed run — Shift already folded
            buf.append(p[1])
            i += 1
            continue
        if kind == "terminate":
            flush_pend()
            flush_buf()
            chips.append("terminate")
            i += 1
            continue
        name = p[1]
        if name in _ORDERED_BUTTONS:
            flush_pend()
            flush_buf()
            if kind == "up":  # unpaired: its down was in an earlier turn
                chips.append(f"release({name})")
                i += 1
                continue
            reps, j = 0, i
            while (j + 1 < n and prims[j] == ("down", name)
                   and prims[j + 1] == ("up", name)):
                reps += 1
                j += 2
                if name != "LMB" or reps == 3:
                    break
            if reps == 0:
                chips.append(chord_prefix() + f"hold({name})")
                i += 1
            else:
                label = ({2: "dblclick", 3: "triclick"}[reps]
                         if name == "LMB" and reps > 1 else _ORDERED_BUTTONS[name])
                chips.append(chord_prefix() + label)
                i = j
            continue
        is_mod = name in _ORDERED_MODS
        if kind == "down":
            shift = bool(_ORDERED_SHIFT & held.keys())
            typing_vetoed = is_mod or (held.keys() - _ORDERED_SHIFT)
            ch = None if typing_vetoed else _ordered_typing_char(name, shift)
            if ch is not None:
                for m in held:  # Shift folded into the char — not a bare tap
                    if held[m] == "new":
                        held[m] = "used"
                buf.append(ch)
                i += 1
                continue
            if is_mod:
                held.setdefault(name, "new")
                i += 1
                continue
            flush_pend()
            flush_buf()
            if i + 1 < n and prims[i + 1] == ("up", name):
                chips.append(chord_prefix() + f"key({name})")
                i += 2
            else:
                chips.append(chord_prefix() + f"down({name})")
                i += 1
            continue
        # kind == "up"
        if is_mod:
            state = held.pop(name, None)
            if state == "new":  # pressed and released with nothing under it
                flush_pend()
                flush_buf()
                chips.append(f"key({name})")
            elif state is None or state == "carried":
                flush_pend()
                flush_buf()
                chips.append(f"up({name})")
            i += 1
            continue
        if (not (held.keys() - _ORDERED_SHIFT)
                and _ordered_typing_char(name, False) is not None):
            i += 1  # release of a typed character (rollover) — absorbed
            continue
        flush_pend()
        flush_buf()
        chips.append(f"up({name})")
    ...
    flush_pend()
    flush_buf()
    for m, state in held.items():  # held into the next turn
        if state == "new":
            chips.append(f"down({m})")
        held[m] = "carried"
    return chips


def parse_ordered_action(
    text: str, held: "dict[str, str] | None" = None
) -> "tuple[str, str] | None":
    ...
    ``+Terminate -Terminate`` chip as the native translation.

    ``held`` is the per-conversation modifier carry-over for ``_ordered_disp``
    (a Shift/Ctrl held across turns keeps affecting the disp chips)."""
    ...
    if dx == 0 and dy == 0 and scroll == 0 and hscroll == 0 and not events:
        ...
        if hscroll:
            head += f" {hscroll}"
        hud = head if not events else head + " ; " + " ".join(f"{s}{n}" for s, n in events)
    d = " · ".join(_ordered_disp(prims, held if held is not None else {})) or "NO_OP"
    if "<think>" in text:
        d = "💭 " + d
    return hud, d
...
class ConversationsDataset(FrameRecordsDataset):
    ...
    def _parse_conversation(self, rec: dict[str, Any]) -> "Segment | None":
        ...
        ordered_held: dict[str, str] = {}  # cross-turn modifier state (disp only)
        for m in rec.get("messages") or []:
            ...
            if role == "system":
                ...
                if native is None:
                    native = parse_ordered_action(action, ordered_held)
                i = len(seg.frames)
