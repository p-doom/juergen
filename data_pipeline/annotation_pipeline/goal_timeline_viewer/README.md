...
```bash
cd data_pipeline/annotation_pipeline/goal_timeline_viewer
...
uv run build_recording_day.py --uploads $UP --recording $REC --out data/$REC/day.json
...
uv run annotator.py serve --data_dir data/$REC --port 8791 --no_prewarm
...
uv run annotator.py index --user 87776db2-8265-4b9a-8199-ba43f1baf479 --tz Europe/Berlin
...
uv run annotator.py index --user 87776db2-8265-4b9a-8199-ba43f1baf479 \
...
uv run annotator.py serve --data_dir $DATA --port 8753   # --verbose to log every range request
```
...
```bash
uv run annotator.py align --data_dir $DATA   # writes $DATA/alignment.json
```
