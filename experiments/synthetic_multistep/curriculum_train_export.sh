...
ORBAX="$OUT/orbax"
HF="$OUT/hf"
LEARNING_RATE="${A[learning_rate]:-1e-4}"
python3 - "$LEARNING_RATE" <<'PY'
import math, sys
try: value = float(sys.argv[1])
except ValueError: raise SystemExit(f"FATAL invalid learning_rate: {sys.argv[1]}")
if not math.isfinite(value) or value <= 0:
    raise SystemExit(f"FATAL learning_rate must be finite and positive: {value}")
PY
...
mkdir -p "$ORBAX"
lr_tag="${LEARNING_RATE//./p}"; lr_tag="${lr_tag//-/m}"
job_tag="${SLURM_JOB_ID:-local}_${BRANCH}_raw_pre_r256_lr${lr_tag}"
JAX_CACHE="/fast/project/HFMI_SynergyUnit/p-doom_shared/franz/jax_cache/curriculum_${job_tag}"
mkdir -p "$JAX_CACHE"
...
uv run --project="$OMX" -- srun python scripts/train_vlm_sft.py \
  ...
  --learning_rate="$LEARNING_RATE" --lr_schedule=wsd --lr_stable_fraction=0.7 \
