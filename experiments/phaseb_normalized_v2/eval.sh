#!/bin/bash
set -euo pipefail

declare -A A
for arg in "$@"; do
  case "$arg" in
    --*=*) key="${arg%%=*}"; key="${key#--}"; A[$key]="${arg#*=}" ;;
    *) echo "FATAL unexpected argument: $arg" >&2; exit 2 ;;
  esac
done
for key in model model_manifest model_stage dataset raw_gold out runtime_dir experiment_dir; do
  [[ -n "${A[$key]:-}" ]] || { echo "FATAL missing --$key" >&2; exit 2; }
done

MODEL="${A[model]}/hf"
MANIFEST="${A[model_manifest]}"
VAL="${A[dataset]}/_normalized/val/chat.jsonl"
RAW_GOLD="${A[raw_gold]}/val/chat.jsonl"
OUT="${A[out]}"
[[ -f "$MODEL/config.json" && -f "$MANIFEST" && -f "$VAL" && -f "$RAW_GOLD" ]] || {
  echo "FATAL normalized-v2 eval input missing" >&2; exit 3;
}
mkdir -p "$OUT"
PY="${A[runtime_dir]}/.venv/bin/python"
"$PY" "${A[experiment_dir]}/../phaseb_oracle_eval.py" \
  --schema normalized --val-chat "$VAL" --raw-gold "$RAW_GOLD" \
  --normalized-gold "$VAL" --out "$OUT/selftest" --model-dir "$MODEL" \
  --model-manifest "$MANIFEST" --model-stage "${A[model_stage]}" --selftest

cache="/fast/project/HFMI_SynergyUnit/p-doom_shared/franz/tmp/phaseb_normalized_v2_vllm_${SLURM_JOB_ID:-local}"
export VLLM_CACHE_ROOT="$cache" TORCHINDUCTOR_CACHE_DIR="$cache/inductor" \
  TRITON_CACHE_DIR="$cache/triton"
export CUDA_HOME=/fast/service/apps/software/CUDA/12.6.0 \
  VLLM_USE_FLASHINFER_SAMPLER=0 VLLM_ATTENTION_BACKEND=FLASH_ATTN
mkdir -p "$VLLM_CACHE_ROOT" "$TORCHINDUCTOR_CACHE_DIR" "$TRITON_CACHE_DIR"
port=$((20000 + (${SLURM_JOB_ID:-1} % 1000)))
url="http://127.0.0.1:${port}/v1"
cd "${A[runtime_dir]}"
setsid --wait uv run --no-sync vllm serve "$MODEL" --host 127.0.0.1 --port "$port" \
  --served-model-name policy --gpu-memory-utilization 0.85 --max-model-len 16384 --enforce-eager \
  >"$OUT/vllm.log" 2>&1 &
pid=$!
shutdown_vllm() {
  kill -TERM -- "-$pid" 2>/dev/null || true
  for _ in $(seq 1 30); do
    kill -0 -- "-$pid" 2>/dev/null || break
    sleep 1
  done
  kill -KILL -- "-$pid" 2>/dev/null || true
  wait "$pid" 2>/dev/null || true
  ! kill -0 -- "-$pid" 2>/dev/null || {
    echo "FATAL vLLM process group survived shutdown" >&2
    return 1
  }
}
trap shutdown_vllm EXIT
"$PY" "${A[experiment_dir]}/../relative_factorial/readiness.py" \
  --base-url "$url" --model policy --grammar move_rel --timeout-s 900 --pid "$pid"
"$PY" "${A[experiment_dir]}/../phaseb_oracle_eval.py" \
  --schema normalized --val-chat "$VAL" --raw-gold "$RAW_GOLD" \
  --normalized-gold "$VAL" --out "$OUT" --base-url "$url" --model policy \
  --model-dir "$MODEL" --model-manifest "$MANIFEST" --model-stage "${A[model_stage]}" \
  --selftest --concurrency 12
shutdown_vllm
trap - EXIT
