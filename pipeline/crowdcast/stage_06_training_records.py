...
#: Written by build_sft_records_from_chat.py beside each split.
...
flags.DEFINE_integer(
    ...
    "when > 0, so an older omegalax checkout without the flag still runs; "
    "per-split counts land under min_length in truncation_stats.json.",
    lower_bound=0,
)
flags.DEFINE_bool(
    "carry_goal_context",
    ...
    "goal-conditioned conversation split at max_length yields standalone "
    "goal-conditioned chunks instead of headless screenshot->action tails. "
    "Budget for the carried header is reserved up front; the stage-05 measure "
    "cache is reused unchanged (the carried text is priced separately at build "
    "time). Forwarded as --carry_goal_context; no-op for other overflow modes. "
    "Per-split truncation_stats.json reports injected_chunks.",
)
flags.DEFINE_bool(
    "plot_vision_tokens",
    True,
    "After each split is built, render its vision-tokens-per-chunk percentile "
    f"CDF to <out>/<split>/{VISION_CDF_FILENAME} from the builder's "
    f"{TOKEN_STATS_FILENAME}. Key percentiles land in manifest.json either way; "
    "only the PNG is skipped when disabled (or matplotlib is unavailable).",
)
...
def _run_split(split: str, src_chat: Path, out_split_dir: Path, cache_path: Path | None) -> dict:
    ...
    cmd = [
        ...
        f"--split={split}",
        ("--carry_goal_context" if FLAGS.carry_goal_context
         else "--nocarry_goal_context"),
        "--overwrite",
    ]
    # Appended only when set, so an omegalax checkout predating --min_length
    # still runs rather than dying on an unknown flag.
...
def main(_) -> None:
    ...
    # -- or, worse, silently emit nothing because no chunk can satisfy both.
    if FLAGS.min_length > FLAGS.max_length:
        raise ValueError(
            f"--min_length={FLAGS.min_length} exceeds --max_length="
            f"{FLAGS.max_length}; no chunk can satisfy both, so every split "
            "would come out empty."
        )
    if FLAGS.carry_goal_context and FLAGS.overflow_mode != "split":
        print(
            f"[stage_records] WARNING: --carry_goal_context is a no-op with "
            f"--overflow_mode={FLAGS.overflow_mode}; it only affects the chunks "
            "a split conversation produces.",
            flush=True,
        )
