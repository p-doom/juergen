...
# Make the ``pipeline`` and ``grammars`` packages importable when run
# directly as a file path, which is how labctl dispatches every stage.
REPO_ROOT = Path(__file__).resolve().parents[2]
if str(REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(REPO_ROOT))

from pipeline.crowdcast.lib import config  # noqa: E402
from pipeline.crowdcast.lib.app_context import (  # noqa: E402
    frame_app_stats,
    load_app_track,
)
