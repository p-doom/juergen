...
def test_typed_controls_lift_to_the_ordered_protocols_existing_events() -> None:
    ...
    action = CODEC.action_from_operations(
        source, geometry=GEOMETRY, cursor=(100, 100)
    )
    line = 'type("a"); down(Return); up(Return); type("b"); down(Tab); up(Tab); type("c")'
    expected = (
        ir.coalesced_type("a"),
        ir.key_down("Return"),
        ir.key_up("Return"),
...
def test_a_click_lifts_and_compiles_with_its_own_button(button: str, token: str) -> None:
    action = CODEC.action_from_operations(
...
def test_episode_cleanup_releases_held_inputs_newest_first() -> None:
    ...
    operations = CODEC.compile(
        "down(ControlLeft); down(RMB)", GEOMETRY, (100, 100)
    )
