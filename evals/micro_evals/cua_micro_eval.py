...
from evals.micro_evals import cua_micro_harvest
from evals.micro_evals import cua_micro_oracle
from evals.micro_evals import cua_micro_wandb
from evals.micro_evals import sampling as sampling_mod
from evals.micro_evals.action_parser import ComputerUseCall, parse_ordered_action_tolerant
from evals.micro_evals.action_parser import OrderedAction as NativeOrderedAction
from evals.micro_evals.cua_micro_action_parser import (
...
from evals.micro_evals.osworld_runtime import (
    _DEFAULT_QCOW2,
    _DEFAULT_QEMU_BIN,
    _EVAL_DIR,
...
import prompts  # the grammar-composed registry (repo root)
from evals.micro_evals.osworld_system_prompts import SYSTEM_PROMPTS
from evals.micro_evals.osworld_vm_client import OSWorldClient
from evals.micro_evals.sampling import SamplingParams
...
#: A `prompts/` registry entry composed over this grammar reads as the same
#: action format. `ordered_events_v3` IS the grammar `cua_ordered_typing_v1`
#: names — same primitives, same `; ` separator, same NO_OP — so a checkpoint
#: trained under `--system-prompt-id ordered_events_v3_goal` in stage 04 is
#: scored here by naming that same id, and the parser does not change.
_GRAMMAR_FORMATS = {"ordered_events_v3": _NATIVE_ORDERED_FORMAT}


def resolve_system_prompt(prompt_id: str) -> tuple[str, str]:
    """`(prompt text, action format)` for a `--system_prompt_id`.

    Two sources, checked in this order:

      * `prompts/` — this repo's registry, where a prompt is a named EDIT of a
        grammar's own spec (goal-conditioned or not, thinking or not). Preferred,
        because its digest is the one stage 04 records in the dataset manifest,
        so a checkpoint and the arm that scores it can be compared.
      * `osworld_system_prompts.SYSTEM_PROMPTS` — the sealed historical prompts.
        Kept because they are what earlier checkpoints were trained under and no
        codec can re-derive them; a prompt that cannot be re-derived is exactly
        the case a registry cannot serve.

    PORT: when this suite becomes an `evals/` family, this is
    `DesktopHarnessConfig.system_prompt_override` + `system_prompt_sha256`, and
    the sealed branch needs an `expect_prompt_mismatch` justification.
    """
    if prompt_id in prompts.PROMPTS:
        named = prompts.get(prompt_id)
        fmt = _GRAMMAR_FORMATS.get(named.grammar)
        if fmt is None:
            raise SystemExit(
                f"--system_prompt_id {prompt_id!r} is composed over grammar "
                f"{named.grammar!r}, which this suite has no parser for "
                f"(known: {sorted(_GRAMMAR_FORMATS)})"
            )
        return named.describe(), fmt
    if prompt_id in SYSTEM_PROMPTS and prompt_id in _PROMPT_FORMATS:
        return SYSTEM_PROMPTS[prompt_id], _PROMPT_FORMATS[prompt_id]
    raise SystemExit(
        f"unsupported --system_prompt_id {prompt_id!r}; choose a registered "
        f"prompt ({sorted(prompts.PROMPTS)}) or a sealed one "
        f"({sorted(_PROMPT_FORMATS)})"
    )
...
def main() -> int:
    ...
    try:
        _, inferred_format = resolve_system_prompt(args.system_prompt_id)
    ...
    system_prompt, _ = resolve_system_prompt(args.system_prompt_id)
