...
from evals.micro_evals.action_parser import ComputerUseCall
...
# Binding contract: action_specs/computer_use_rel_step_v1.json beside this file
# (movement scales/directions, scroll steps, max tool calls, typing char cap).
#
# It travels WITH the parser rather than living in the data pipeline: this
# grammar has no codec in `grammars/`, so the JSON is its only specification and
# splitting the two across packages is how they drift.
# PORT: a `grammars/` codec would replace both, and `_support.render_tool_prompt`
# already builds this exact `<tools>` envelope from a codec's docstrings.
# --------------------------------------------------------------------------

_REL_STEP_SPEC_PATH = (
    Path(__file__).resolve().parent / "action_specs" / "computer_use_rel_step_v1.json"
)
