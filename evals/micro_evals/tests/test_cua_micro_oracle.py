...
from evals.micro_evals.action_parser import parse_ordered_action_tolerant
from evals.micro_evals.cua_micro_eval import (
...
# `slurm/` is NOT tracked in this repository — it is a local recipe tree beside
# the checkout — so these suites are present for whoever has it and absent in a
# clean clone or a dispatched run. Skipped rather than failed: their absence says
# nothing about the harness, and a red suite that is red for everyone stops being
# read. `$JUERGEN_SLURM_ROOT` points at a tree in a non-default location.
_SLURM_ROOT = Path(
    os.environ.get("JUERGEN_SLURM_ROOT")
    or Path(__file__).resolve().parents[3] / "slurm"
)
_V22 = _SLURM_ROOT / "dev/alfred/berlin/labctl/recipes/eval/osworld_freeroll_v22_claude"
_needs_v22 = pytest.mark.skipif(
    not _V22.is_dir(), reason=f"oracle suites not present at {_V22} (untracked slurm/)"
)
