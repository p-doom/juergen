...
class SuiteContractTests(unittest.TestCase):
    ...
    @pytest.mark.xfail(
        strict=True,
        reason="asserts a suite shape cua_micro_tasks.json no longer has: 0 atomic "
        "typing tasks, not >=2. Stale on feat/cua-micro-evals too. The suite is "
        "frozen for calibration, so whether the suite lost typing coverage or the "
        "contract moved is a question for its author -- not something to settle by "
        "editing either side.",
    )
    ...
    @pytest.mark.xfail(
        strict=True,
        reason="asserts exactly 6 tasks carry turns; cua_micro_tasks.json ships 8. "
        "Stale on feat/cua-micro-evals too -- see the sibling xfail.",
    )
    ...
    @pytest.mark.xfail(
        strict=True,
        reason="asserts every multiturn turn is a wildcard against one shared "
        "verifier; cua_micro_tasks.json ships turns that are not. Stale on "
        "feat/cua-micro-evals too -- see the sibling xfail.",
    )
