...
def _job_ports(*, job_mod: int, n_slots: int, sglang_port: int | None) -> list[int]:
    """Every port this job will bind: one vm/vnc pair per slot, plus sglang.

    ``sglang_port`` is None when the run drives an endpoint it did not start
    (``--sglang_url``) or none at all (harvest mode) -- nothing of ours binds
    it then, so it stays out of the preflight.
    """
    ports = [5000 + job_mod + i for i in range(n_slots)]
    ports += [5900 + job_mod + i for i in range(n_slots)]
    if sglang_port is not None:
        ports.append(sglang_port)
    return ports
...
def _preflight_ports(ports: list[int], *, job_mod: int) -> None:
    ...
    port, as a whole run silently scored against the OTHER job's model (see
    _assert_serving_our_model).
    """
...
def _assert_serving_our_model(sglang_url: str, *, api_key: str, model_path: str) -> None:
    """Verify the server that answered the health probe serves OUR weights.

    The readiness probe is just an HTTP GET on a port, so a *foreign* sglang
    already bound to it answers 200 and every completion is then generated by
    someone else's model, at full speed and with plausible-looking output --
    a run of ours scored 40 attempts that way (job 143034: our own server
    logged "[Errno 98] address already in use" and exited, the probe passed
    in 10s instead of the usual ~150s, and the replies carried a thinking
    model's chain-of-thought the checkpoint could not produce). Neither the
    port preflight nor _wait_for's proc.poll() catches that: the preflight
    runs before the squatter appears, and our process is still alive (loading
    weights) when the foreign server answers.

    sglang reports ``served_model_name`` -- which defaults to the model path
    it was launched with -- from /v1/models, so comparing that against the
    checkpoint we asked for identifies the server behind the port. A server
    that cannot be queried, or that reports no ids, only warns: not every
    build exposes the endpoint, and refusing to run would be worse than the
    (now much narrower) risk it guards.
    """
    try:
        r = requests.get(
        ...
        r.raise_for_status()
        served = [str(m.get("id", "")) for m in (r.json().get("data") or [])]
    except (requests.RequestException, ValueError, AttributeError) as exc:
        _LOGGER.warning(
            "could not verify which model %s serves (%s) -- continuing unverified",
            sglang_url,
            exc,
        )
        return
    if not served:
        _LOGGER.warning("%s reported no served model ids -- continuing unverified", sglang_url)
        return
    if model_path not in served:
        raise SystemExit(
            f"sglang port hijack: {sglang_url} serves {served}, not the checkpoint "
            f"we launched ({model_path}). Another job's server holds this port, so "
            "every completion would be generated by ITS model -- kill the stale "
            "server or resubmit for a different job id."
        )
    _LOGGER.info("verified %s serves %s", sglang_url, model_path)
...
def main() -> int:
    ...
    # None whenever this run does not start a server of its own -- an endpoint
    # passed with --sglang_url is someone else's to bind, and neither harvest
    # mode nor --validate_setups_only ever reaches the launch below.
    _own_sglang_port: int | None = None
    if args.mode != "harvest" and not args.validate_setups_only and not args.sglang_url:
        _own_sglang_port = args.sglang_port if args.sglang_port != 30000 else 30000 + job_mod
    ...
    # or, on the sglang port, a full run of completions from a foreign model.
    _n_slots = 1 if args.validate_setups_only else args.vms_per_sglang
    _preflight_ports(
        _job_ports(job_mod=job_mod, n_slots=_n_slots, sglang_port=_own_sglang_port),
        job_mod=job_mod,
    )
    ...
    # Stays None in the two branches that never start a server; the end-of-run
    # liveness check below only applies to a server we own.
    ...
    if args.mode == "harvest":
        ...
        assert _own_sglang_port is not None  # set together with the branch we are in
        sglang_port = _own_sglang_port
        ...
        # The probe above only proves *something* answers on this port, so ask
        # it whose weights it holds before scoring a single attempt.
        _assert_serving_our_model(
            sglang_url, api_key=args.sglang_api_key, model_path=args.model_path
        )
    ...
    # A server that died mid-run leaves its port free for the next job to
    # take, and every attempt after that point would have been answered by
    # whatever bound it -- so results from a run whose own sglang is gone are
    # not attributable to this checkpoint. Loud and result-less on purpose:
    # tainted numbers that look fine are how a foreign model gets scored.
    if sglang_proc is not None and sglang_proc.poll() is not None:
        raise SystemExit(
            f"our sglang exited mid-run (rc={sglang_proc.returncode}); attempts after "
            "that point may have been served by another job on the same port -- "
            "see sglang.log and rerun."
        )
