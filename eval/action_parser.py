...
# --------------------------------------------------------------------------
# Official Qwen3-VL computer-use cookbook tool calls (baseline condition b).
#
# Ported from yll/cua-micro-evals so an UNTRAINED Qwen3-VL can be scored in
# its own schema: the validation and strict parse are that branch's, the
# adapter is not. Coordinates are ABSOLUTE on the cookbook's 0-1000 grid, so
# the adapter emits our own absolute ``move_to`` (pixels) instead of the
# branch's relative ``move`` against a tracked cursor — no cursor tracking, no
# drift, and the dispatch path is the one ordered_events_v4_abs already uses.
# --------------------------------------------------------------------------

QWEN3VL_GRID = 1000
QWEN3VL_SCROLL_UNITS_PER_NOTCH = 100
QWEN3VL_ANSWER_STATUS = "answer"
...
def _validate_qwen3vl_cua_arguments(arguments: dict, *, where: str) -> None:
    ...
    A ``coordinate`` is REQUIRED by mouse_move and left_click_drag and validated
    wherever else it appears: the schema documents the click families as clicking
    at a coordinate and this adapter honours that, so an out-of-grid click cannot
    be waved through to be silently clipped."""
    ...
    if action in {"mouse_move", "left_click_drag"} or "coordinate" in arguments:
        ...
        if (
            ...
            or any(not 0 <= float(value) <= QWEN3VL_GRID for value in coordinate)
        ):
            raise ValueError(
                f"{where}: coordinate must be two numbers on the 0..{QWEN3VL_GRID} grid"
            )
    if action in {"scroll", "hscroll"}:
...
def qwen3vl_grid_to_pixel(
    coordinate: Sequence[float], screen: Sequence[int],
) -> tuple[int, int]:
    """One cookbook 0-1000 coordinate as an on-screen pixel, clipped to the screen."""
    if len(coordinate) != 2 or len(screen) != 2:
        raise ValueError(f"need (x,y) on a (w,h) screen, got {coordinate!r} {screen!r}")
    width, height = int(screen[0]), int(screen[1])
    if width <= 0 or height <= 0:
        raise ValueError(f"screen must be positive, got {screen!r}")
    return (
        max(0, min(width - 1, round(float(coordinate[0]) * width / QWEN3VL_GRID))),
        max(0, min(height - 1, round(float(coordinate[1]) * height / QWEN3VL_GRID))),
    )


def qwen3vl_scroll_notches(pixels: float) -> int:
    """A cookbook scroll amount as wheel notches, sign preserved.

    The schema calls the argument ``pixels`` on a 1000-unit screen while every
    dispatch path we own counts wheel notches, so the two are not the same unit
    and passing the number through would turn a small scroll into hundreds of
    notches. ``QWEN3VL_SCROLL_UNITS_PER_NOTCH`` is that conversion, and any
    nonzero request keeps at least one notch so a small scroll still scrolls."""
    amount = float(pixels)
    notches = round(amount / QWEN3VL_SCROLL_UNITS_PER_NOTCH)
    if notches == 0 and amount != 0:
        return 1 if amount > 0 else -1
    return notches


def qwen3vl_native_to_ordered(
    calls: Sequence[ComputerUseCall], screen: Sequence[int],
) -> OrderedAction:
    """Adapt official absolute-grid Qwen3-VL calls to dispatchable VM primitives.

    Coordinates arrive absolute, so they become ``move_to`` PIXELS here and need
    no denormalization afterwards. Three cookbook actions have no primitive of
    their own and are expanded or surfaced instead of dropped:

    - a click carrying a ``coordinate`` becomes ``move_to(target)`` then the
      click, because the schema documents the click families as clicking AT a
      coordinate; a click without one still clicks where the cursor is.
    - ``left_click_drag`` becomes ``button_down(left); move_to(target);
      button_up(left)`` — press where the cursor already is, drag, release.
    - ``terminate`` keeps its ``status`` and is NEVER dispatched: the rollout
      loop stops on it, exactly like the v4 ``TERMINATE`` line.
    - ``answer`` keeps its text as a ``terminate``-shaped stop with status
      ``answer``, so a model that answers instead of acting ends its episode and
      is scored by the verifier rather than silently continuing.
    """
    ...
    for call in calls:
        ...
        if action == "mouse_move":
            x, y = qwen3vl_grid_to_pixel(arguments["coordinate"], screen)
            primitives.append(OrderedPrimitive(kind="move_to", x=x, y=y))
        elif action in _CUA_V4_CLICKS:
            button, count = _CUA_V4_CLICKS[action]
            if arguments.get("coordinate") is not None:
                x, y = qwen3vl_grid_to_pixel(arguments["coordinate"], screen)
                primitives.append(OrderedPrimitive(kind="move_to", x=x, y=y))
            ...
            x, y = qwen3vl_grid_to_pixel(arguments["coordinate"], screen)
            primitives.extend((
                OrderedPrimitive(kind="button_down", name="left"),
                OrderedPrimitive(kind="move_to", x=x, y=y),
                OrderedPrimitive(kind="button_up", name="left"),
            ))
        elif action == "type":
            ...
            primitives.append(OrderedPrimitive(
                kind="scroll", dx=0, dy=qwen3vl_scroll_notches(arguments["pixels"]),
            ))
        elif action == "hscroll":
            primitives.append(OrderedPrimitive(
                kind="scroll", dx=qwen3vl_scroll_notches(arguments["pixels"]), dy=0,
            ))
        elif action == "wait":
            ...
            primitives.append(OrderedPrimitive(
                kind="terminate", status=QWEN3VL_ANSWER_STATUS, text=arguments["text"],
            ))
        else:
            raise AssertionError(f"unhandled Qwen3-VL action {action!r}")
    return OrderedAction(
        primitives=tuple(primitives), no_op=all(p.kind == "wait" for p in primitives),
    )
