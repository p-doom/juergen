...
pointed at the **stage 03 sampler output** instead of raw mp4 + keylogs. stage 03 has
...
transcode, no raw-vs-realigned dual clock — just browse the trajectory stage 03
produced, one frame + one action per step.

Input contract (one JSON object per line, the ``frame_records.jsonl`` schema
that stage 01 emits and stage 03 is expected to reproduce)::
...
01 and stage 03 stream ``chat.jsonl``/segments in order, so this holds). Any extra
...
Run::
    ...
    cd .../data_pipeline
    uv run python tooling/viewers/visualize_frame_records.py \
...
# Make the ``pipeline.crowdcast`` package importable when run directly
...
def _is_black(mean_luma: Any, frac_dark: Any) -> bool:
    ...
    ``sample_frames_actions._is_black`` so both views flag exactly the frames stage 03
    would drop under default settings — the master reads its own manifest, the
    stage 03 sample cross-references the master's. Frames without metrics (older
...
class ConversationsDataset(FrameRecordsDataset):
    ...
    refs into the SAME stage-01a master store the stage 03 sample used, so frame
...
class FramesMasterDataset:
    ...
    stage 03 sampler). So this view shows frames + timing only; the action HUD stays
        ...
        with manifest.open() as f:
            for line in f:
                ...
                frames.append({
                    ...
                    # what the stage 03 sampler drops with --drop-black-frames on.
...
async function loadSegments(){
  ...
  if(MODE==='frames_master' && info.has_alignment){
    ...
    note.textContent=`raw frames-master (01a), master_fps=${info.master_fps||'?'} — no keylog linked, so no actions. Pass --clips-manifest, or run the stage 03 sampler.`;
    note.classList.add('show');
...
// from its own luma metrics; stage 03 sample: cross-referenced from the master).
...
def resolve_jsonl_paths(dataset: Path) -> list[Path]:
    ...
    Handles both shapes:
      ...
      * the stage-stage 03 annotate layout — one file per segment at
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help="one or more datasets, each a stage 03 output dir (or frame_records.jsonl "
