...
for the ordered grammar directly (``grammars.describe("ordered_events_v3")``,
...
Run::
    ...
    cd .../data_pipeline
    uv run python tooling/viewers/visualize_ordered_action_distribution.py \
...
# The grammar's OWN codec, which is what the eval harness and the RL rollout
# parse with — so this viewer, the emitter and the reader all have one
# implementation of the grammar rather than three that agree until they do not.
# (It replaces `eval/action_parser.parse_ordered_action`, a second parser that
# existed only because the codec did not.)
import grammars  # noqa: E402
from grammars.ordered_events_v3 import CODEC as ORDERED_EVENTS_V3_CODEC  # noqa: E402


def parse_ordered_line(line: str):
    """One action line -> its parsed action. Raises on anything malformed."""
    return ORDERED_EVENTS_V3_CODEC.parse(line)
        ...
        try:
            if route == "/":
                ...
                self._send_json({
                    ...
                    "grammar": grammars.describe("ordered_events_v3").strip("\n"),
                    "format": V3_FORMAT,
                    "turn_fields": [
