...
Typical flow (``--script`` is required: this file's PEP 723 header above is what
supplies av/msgpack, and any other form — notably ``uv run python annotator.py``
— makes uv discard the inline metadata and fall back to pyproject discovery):
  uv run --script annotator.py index --user <UID>                        # survey the days
  uv run --script annotator.py index --user <UID> --date <YYYY-MM-DD> --out <DATA>/day.json
  uv run --script annotator.py prewarm --data_dir <DATA>                 # optional, on a compute node
  uv run --script annotator.py serve   --data_dir <DATA> --port 8753     # auto-builds alignment + warms
